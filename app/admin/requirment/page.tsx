...
const getColorClass = (color: string) => {
  const colors: Record<string, string> = {
    ...
    orange: "bg-orange-100 text-orange-700 border-orange-200 dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-700",
    red: "bg-red-100 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-700",
    emerald: "bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-700",
    pink: "bg-pink-100 text-pink-700 border-pink-200 dark:bg-pink-900/30 dark:text-pink-400 dark:border-pink-700",
    indigo: "bg-indigo-100 text-indigo-700 border-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-400 dark:border-indigo-700",
  };
  return colors[color] || "bg-gray-100 text-gray-700 border-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600";
};
...
export default function AdmissionManagement() {
  ...
  // ===== SAFE FILTERING (FIX: undefined check) =====
  const filteredRequirements = requirements.filter((item) => {
    const title = item.title || "";
    const level = item.level || "";
    const search = searchTerm.toLowerCase();
    return title.toLowerCase().includes(search) || level.toLowerCase().includes(search);
  });

  const filteredEligibility = eligibility.filter((item) => {
    const level = item.level || "";
    return level.toLowerCase().includes(searchTerm.toLowerCase());
  });
  ...
  const filteredGuidelines = guidelines.filter((item) => {
    ...
    const search = searchTerm.toLowerCase();
    return step.toLowerCase().includes(search) || description.toLowerCase().includes(search);
  });
  ...
  const handleEditSave = () => {
    if (modalType === "requirement") {
      setRequirements(requirements.map((item) => (item.id === editFormData.id ? editFormData : item)));
    } else if (modalType === "eligibility") {
      setEligibility(eligibility.map((item) => (item.id === editFormData.id ? editFormData : item)));
    } else if (modalType === "document") {
      setDocuments(documents.map((item) => (item.id === editFormData.id ? editFormData : item)));
    } else if (modalType === "guideline") {
      setGuidelines(guidelines.map((item) => (item.id === editFormData.id ? editFormData : item)));
    }
  ...
  const handleAddSave = () => {
    // Safely compute next ID
    let maxId = 0;
    if (modalType === "requirement") maxId = requirements.reduce((max, item) => Math.max(max, item.id), 0);
    else if (modalType === "eligibility") maxId = eligibility.reduce((max, item) => Math.max(max, item.id), 0);
    else if (modalType === "document") maxId = documents.reduce((max, item) => Math.max(max, item.id), 0);
    else if (modalType === "guideline") maxId = guidelines.reduce((max, item) => Math.max(max, item.id), 0);
    const newId = maxId + 1;
    ...
    alert("নতুন আইটেম যোগ করা হয়েছে");
  };
    ...
    <div>
      <div className="flex flex-wrap gap-4 items-center justify-between mb-6">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent dark:bg-gray-800 dark:text-white"
          />
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredRequirements.map((item) => (
          <div
            key={item.id}
            className="group bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 hover:border-green-200 dark:hover:border-green-700 border border-transparent dark:border-gray-700"
          >
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-3">
                <div className="text-2xl">{item.icon}</div>
                <div>
                  <h3 className="font-semibold text-gray-800 dark:text-gray-100 group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors text-sm sm:text-base">
                    {item.title}
                  </h3>
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full border ${getColorClass(item.color)} inline-block mt-1`}
                  >
                    {item.level}
                  </span>
                </div>
              ...
              <div className="flex gap-1">
                <button
                  onClick={() => openViewModal(item, "requirement")}
                ...
                <button
                  onClick={() => openEditModal(item, "requirement")}
                ...
                <button
                  onClick={() => openDeleteModal(item, "requirement")}
            ...
            <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-2">{item.description}</p>
            <div className="mt-3 flex flex-wrap gap-1">
              {/* SAFE SLICE: ensure details is an array */}
              ...
              {item.details && item.details.length > 3 && (
                <span className="text-[10px] text-gray-400 dark:text-gray-500">
                  +{toBanglaNumber(item.details.length - 3)} আরো
                </span>
              )}
            </div>
          </div>
      ...
      {filteredRequirements.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 dark:text-gray-400">কোন প্রয়োজনীয়তা পাওয়া যায়নি</p>
        </div>
      )}
    </div>
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredEligibility.map((item) => (
          <div
            key={item.id}
            className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 hover:border-green-200 dark:hover:border-green-700 border border-transparent dark:border-gray-700"
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
              <h3 className="font-semibold text-gray-800 dark:text-gray-100 text-sm sm:text-base">{item.level}</h3>
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(item.status)}`}>
                {item.status}
              </span>
            </div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">আসন: {toBanglaNumber(item.seats)}</p>
            <ul className="mt-3 space-y-1">
              {(item.criteria || []).slice(0, 3).map((c, idx) => (
                <li key={idx} className="flex items-start gap-2 text-xs text-gray-600 dark:text-gray-400">
                  <ShieldCheck className="h-3.5 w-3.5 text-green-500 dark:text-green-400 flex-shrink-0 mt-0.5" />
                  <span>{c}</span>
                </li>
              ))}
              {item.criteria && item.criteria.length > 3 && (
                <li className="text-xs text-gray-400 dark:text-gray-500">+{toBanglaNumber(item.criteria.length - 3)} আরো</li>
              )}
            </ul>
            <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-2">
              <button
                onClick={() => openViewModal(item, "eligibility")}
                className="p-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-all duration-300 hover:scale-110"
              >
                <Eye className="h-4 w-4" />
              </button>
              <button
                onClick={() => openEditModal(item, "eligibility")}
                className="p-1.5 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/50 transition-all duration-300 hover:scale-110"
              >
                <Edit className="h-4 w-4" />
              </button>
              <button
                onClick={() => openDeleteModal(item, "eligibility")}
                className="p-1.5 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/50 transition-all duration-300 hover:scale-110"
              >
                <Trash2 className="h-4 w-4" />
              </button>
      ...
      {filteredEligibility.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 dark:text-gray-400">কোন যোগ্যতা পাওয়া যায়নি</p>
        </div>
      )}
    </div>
  );
    ...
    <div>
        ...
        {filteredDocuments.map((item) => (
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-gray-800 dark:text-gray-100 text-sm sm:text-base">{item.category}</h3>
            </div>
            <ul className="space-y-1.5">
              {(item.items || []).slice(0, 4).map((it, idx) => (
                <li key={idx} className="flex items-start gap-2 text-xs">
                  {it.required ? (
                    <CheckCircle className="h-3.5 w-3.5 text-green-500 dark:text-green-400 flex-shrink-0 mt-0.5" />
                  ) : (
                    <MinusCircle className="h-3.5 w-3.5 text-gray-400 dark:text-gray-500 flex-shrink-0 mt-0.5" />
                  )}
                  <div>
                    <p className="text-gray-700 dark:text-gray-300">{it.name}</p>
                    <p className="text-[10px] text-gray-400 dark:text-gray-500">{it.format}</p>
                  </div>
                </li>
              ))}
              {item.items && item.items.length > 4 && (
                <li className="text-xs text-gray-400 dark:text-gray-500">+{toBanglaNumber(item.items.length - 4)} আরো</li>
              )}
            </ul>
            <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-2">
              <button
                onClick={() => openViewModal(item, "document")}
                className="p-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-all duration-300 hover:scale-110"
              >
                <Eye className="h-4 w-4" />
              </button>
              <button
                onClick={() => openEditModal(item, "document")}
                className="p-1.5 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/50 transition-all duration-300 hover:scale-110"
              >
                <Edit className="h-4 w-4" />
              </button>
              <button
                onClick={() => openDeleteModal(item, "document")}
                className="p-1.5 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/50 transition-all duration-300 hover:scale-110"
              >
                <Trash2 className="h-4 w-4" />
              </button>
      ...
      {filteredDocuments.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 dark:text-gray-400">কোন নথিপত্র পাওয়া যায়নি</p>
        </div>
      )}
    </div>
  );
    ...
    <div>
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-800 dark:text-gray-100 text-sm sm:text-base">{item.step}</h3>
                  ...
                  <ul className="mt-2 space-y-1">
                    {(item.details || []).slice(0, 3).map((d, idx) => (
                      <li key={idx} className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                        <ChevronDown className="h-3 w-3 text-green-500 dark:text-green-400 -rotate-90 flex-shrink-0" />
                        <span>{d}</span>
                      </li>
                    ))}
                    {item.details && item.details.length > 3 && (
                      <li className="text-xs text-gray-400 dark:text-gray-500">+{toBanglaNumber(item.details.length - 3)} আরো</li>
                    )}
                  </ul>
                </div>
              </div>
              <div className="flex gap-1">
                <button
                  onClick={() => openViewModal(item, "guideline")}
                  className="p-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-all duration-300 hover:scale-110"
                >
                  <Eye className="h-4 w-4" />
                </button>
                <button
                  onClick={() => openEditModal(item, "guideline")}
                  className="p-1.5 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/50 transition-all duration-300 hover:scale-110"
                >
                  <Edit className="h-4 w-4" />
                </button>
                <button
                  onClick={() => openDeleteModal(item, "guideline")}
                  className="p-1.5 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/50 transition-all duration-300 hover:scale-110"
                >
                  <Trash2 className="h-4 w-4" />
                </button>
      ...
      {filteredGuidelines.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 dark:text-gray-400">কোন নির্দেশিকা পাওয়া যায়নি</p>
        </div>
      )}
    </div>
  );
    ...
    if (modalType === "requirement") {
      ...
      content = (
        <div className="space-y-3 text-gray-700 dark:text-gray-300">
          <p>
            <strong>শিরোনাম:</strong> {item.title}
          </p>
          <p>
            <strong>বিবরণ:</strong> {item.description}
          </p>
          <p>
            <strong>স্তর:</strong> {item.level}
          </p>
          <p>
            <strong>আইকন:</strong> {item.icon}
          </p>
          <p>
            <strong>রং:</strong> {item.color}
          </p>
          <p>
      ...
      content = (
        <div className="space-y-3 text-gray-700 dark:text-gray-300">
          ...
          <p>
            <strong>আসন:</strong> {toBanglaNumber(item.seats)}
          </p>
          <p>
        ...
        <div className="space-y-3 text-gray-700 dark:text-gray-300">
          ...
          <ul className="list-disc list-inside space-y-1 text-sm">
            {(item.items || []).map((it: any, idx: number) => (
              <li key={idx}>
                {it.name} ({it.format}) {it.required ? "★প্রয়োজনীয়" : "(ঐচ্ছিক)"}
              </li>
            ))}
          </ul>
        </div>
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          <button
            onClick={() => setViewModalOpen(false)}
            className="mt-4 w-full bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 py-2 rounded-xl font-medium hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
          >
            বন্ধ করুন
          </button>
        </div>
    ...
    if (modalType === "requirement") {
      fields = (
        <>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">স্তর</label>
            <input
              name="level"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">আইকন</label>
            <input
              name="icon"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">রং</label>
            <select
              name="color"
              value={editFormData.color || "green"}
              onChange={handleChange}
              className="w-full px-3 py-2 border rounded-xl dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            >
              ...
              <option value="emerald">পান্না</option>
              <option value="pink">গোলাপি</option>
              <option value="indigo">নীলাভ</option>
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">বিস্তারিত (কমা দিয়ে আলাদা করুন)</label>
            <input
              name="details"
              value={editFormData.details ? editFormData.details.join(", ") : ""}
              onChange={(e) =>
                setEditFormData({
                  ...editFormData,
                  details: e.target.value.split(",").map((s) => s.trim()),
                })
              }
              className="w-full px-3 py-2 border rounded-xl dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            />
      ...
      fields = (
        <>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">আসন</label>
            <input
              name="seats"
              type="number"
          ...
          <div>
            ...
            <select
              name="status"
              value={editFormData.status || "খোলা"}
              onChange={handleChange}
              className="w-full px-3 py-2 border rounded-xl dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            >
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">শর্তাবলী (কমা দিয়ে আলাদা করুন)</label>
            <input
              name="criteria"
              value={editFormData.criteria ? editFormData.criteria.join(", ") : ""}
              onChange={(e) =>
                setEditFormData({
                  ...editFormData,
                  criteria: e.target.value.split(",").map((s) => s.trim()),
                })
              }
              className="w-full px-3 py-2 border rounded-xl dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            />
      ...
      fields = (
        <>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">আইটেম (JSON ফরম্যাটে)</label>
            <textarea
              name="items"
              value={JSON.stringify(editFormData.items || [], null, 2)}
              onChange={(e) => {
                try {
                  setEditFormData({ ...editFormData, items: JSON.parse(e.target.value) });
                } catch (e) {}
              }}
              rows={5}
              className="w-full px-3 py-2 border rounded-xl dark:bg-gray-700 dark:border-gray-600 dark:text-white font-mono text-xs"
            />
          </div>
        </>
      );
      ...
      fields = (
        <>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">ধাপ</label>
            <input
              name="step"
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-2xl w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          <button
            ...
            className="mt-4 w-full bg-gradient-to-r from-green-500 to-green-600 text-white py-2.5 rounded-xl font-medium hover:from-green-600 hover:to-green-700 transition-all duration-300 flex items-center justify-center gap-2"
          >
  ...
  const renderAddModal = () => {
    ...
    if (modalType === "requirement") {
      fields = (
        <>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">আইকন (ইমোজি)</label>
            <input
              name="icon"
          ...
          <div>
            ...
            <input
              name="details"
              value={addFormData.details ? addFormData.details.join(", ") : ""}
              onChange={(e) =>
                setAddFormData({
                  ...addFormData,
                  details: e.target.value.split(",").map((s) => s.trim()),
      ...
      fields = (
        <>
          ...
          <div>
            ...
            <input
              name="criteria"
              value={addFormData.criteria ? addFormData.criteria.join(", ") : ""}
              onChange={(e) =>
                setAddFormData({
                  ...addFormData,
                  criteria: e.target.value.split(",").map((s) => s.trim()),
      ...
      fields = (
        <>
          ...
          <div>
            ...
            <textarea
              name="items"
              value={JSON.stringify(addFormData.items || [{ name: "", required: true, format: "" }], null, 2)}
              onChange={(e) => {
                try {
                  setAddFormData({ ...addFormData, items: JSON.parse(e.target.value) });
                } catch (e) {}
              }}
              rows={5}
              className="w-full px-3 py-2 border rounded-xl dark:bg-gray-700 dark:border-gray-600 dark:text-white font-mono text-xs"
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-2xl w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold text-slate-800 dark:text-white">নতুন যোগ করুন</h3>
            <button
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-sm w-full p-6 shadow-2xl">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-6">
            ...
            <span className="font-semibold text-slate-800 dark:text-white">
              {selectedItem.title || selectedItem.level || selectedItem.step || selectedItem.category}
            </span>{" "}
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300"
            >
              বাতিল
            </button>
            <button
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              ভর্তি সংক্রান্ত প্রয়োজনীয়তা, যোগ্যতা, নথিপত্র ও নির্দেশিকা পরিচালনা করুন
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">প্রয়োজনীয়তা</p>
              <p className="text-2xl font-bold text-slate-800 dark:text-white mt-1">{toBanglaNumber(totalRequirements)}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">যোগ্যতা</p>
              <p className="text-2xl font-bold text-slate-800 dark:text-white mt-1">{toBanglaNumber(totalEligibility)}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">নথিপত্র</p>
              <p className="text-2xl font-bold text-slate-800 dark:text-white mt-1">{toBanglaNumber(totalDocuments)}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">নির্দেশিকা</p>
              <p className="text-2xl font-bold text-slate-800 dark:text-white mt-1">{toBanglaNumber(totalGuidelines)}</p>
            </div>
